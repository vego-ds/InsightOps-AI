...
def test_missing_openrouter_api_key_returns_deterministic_fallback() -> None:
    provider = OpenRouterNarrativeProvider(AppSettings(openrouter_api_key=None))
...
def test_malformed_openrouter_response_falls_back_deterministically() -> None:
    provider = OpenRouterNarrativeProvider(AppSettings(openrouter_api_key="test-key"))
    ...
    assert any(
        "Malformed provider response" in warning for warning in narrative.warnings
    )
...
def test_provider_request_failure_falls_back_deterministically() -> None:
    ...
    assert narrative.mode == "openrouter_fallback"
    assert any("OpenRouter provider error" in warning for warning in narrative.warnings)
